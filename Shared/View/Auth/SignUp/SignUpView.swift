...
struct SignUpView: View {
    ...
    var body: some View {
        if isShowProfile {
            UserProfileScreenView(image: "en_gedi_israel", title: "onboarding_user_profile_title".localized(userLanguage), detail: "May we have some Information?", bgColor: Color("PrimaryColor2"), emailFromSignUp: signUpViewModel.email, displayNameFromSignUp: signUpViewModel.displayName, isFromSettings: false)
                .transition(.scale)
        }else{
            ZStack {
                ScrollView {
                    VStack {
                        ...
                        VStack(spacing: 20, content: {
                            ...
                            VStack(alignment: .leading) {
                                Text("Family Name".localized(userLanguage))
                                  .font(.custom(.inriaSansRegular, size: 18))
                                  .foregroundColor(Color(red: 0.59, green: 0.59, blue: 0.59))
                                HStack {
                                    Image("mail 1")
                                    TextField("Enter Family Name".localized(userLanguage) ,text: $signUpViewModel.familyName)
                                        .padding(.vertical)
                                        .padding(.horizontal, 4)
                                }
                                .padding()
                        ...
                        Button {
                            ...
                            signUpViewModel.signUp { token in
                                ...
                                sessionManager.displayName = signUpViewModel.displayName
                                isShowProfile = true
                            }
                        } label: {
